...
function AddVendor() {
  ...
  const vendorPOST = async() =>{
    if(!postdatavendor.vendor_name || !postdatavendor.vendor_id || !postdatavendor.gst_no || !postdatavendor.email || !postdatavendor.contact_no || !postdatavendor.address || !postdatavendor.vendor_type || !postdatavendor.amount_payable){
  ...
  return <div className='post_vendor_container'>
      <div className="post_vendor_main">
        ...
        <label htmlFor="">Contact NO</label>
          ...
          <input onChange={inputadd} type="text" name="amount_payable" id="" />
        </div>
        </div>
        <div style={{ display: "flex", justifyContent: "center" }}><Button
              
              style={{ padding:"15px 50px", marginTop: "30px", display: "block" }}
              variant="contained"
              color="success"
              onClick={vendorPOST}
            >
